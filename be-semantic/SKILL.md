...
## Step 3〜5：フェイクデータ生成 → 観察 → JSONスキーマ

**Semantic-Ex method で行う。Step 3〜5 を始める前に、必ず `semantic-ex/SKILL.md` を読む**。be-semantic と semantic-ex は同じプラグイン (`be-framework-skills`) 同階層に同梱されており、両方ロードされている前提。Phase 1（Experience）〜 Phase 3（Constraints）の具体的手順・観察軸・スキーマ生成ルール・ファイル命名はすべて `semantic-ex/SKILL.md` が正本。

**並列化のヒント**: ALPS 確定後、Phase 1(Fake 生成)と Phase 3(Schema 草案)は **並列実行できる**。両方揃ってから Phase 2(観察)→ 合意へ進む。sub-agent を使うなら同時起動が有効。

### be-semantic 文脈での追加要件

**Phase 4（確認）のモード別挙動**:

- **インタラクティブモード**: `semantic-ex` Phase 4 のチェックボックスをユーザーに提示し、**すべての項目に返答を得てから** 次に進む。勝手に決めない
- **自走モード**: 提示はするが返答待ちはしない。観察根拠を `$comment` に残して進める。ユーザーは事後に `design/schema/` を見て指摘できる

**アウトプット（パスは semantic-ex と同一）**:

- `design/fake/data-50.json`
- `design/fake/observations.md`
